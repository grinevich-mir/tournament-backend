...
export class HttpClient {
    ...
    private readonly retryConfig: Partial<RetryOptions> = {
        ...
        onRetry: (err, retryCount) => Logger.warn(`Crash API call failed, retry ${retryCount}/${this.maxRetries}: ${err.message}`),
        onSuccess: (retryCount, totalTime) => {
            if (retryCount === 0)
                return;

            Logger.error(`Crash API call succeeded after ${retryCount} ${retryCount === 1 ? 'retry' : 'retries'} and ${totalTime} ms.`);
        }
    };
    ...
    private async getParameter(name: string, decrypt: boolean = false): Promise<string> {
        return this.parameterStore.get(`/${Config.stage}/integration/crash/${name}`, decrypt, true);
    }
