...
export class HttpClient {
    ...
    private readonly retryConfig: Partial<RetryOptions> = {
        ...
        onRetry: (err, retryCount) => Logger.warn(`HiLo API call failed, retry ${retryCount}/${this.maxRetries}: ${err.message}`),
        onSuccess: (retryCount, totalTime) => {
            if (retryCount === 0)
                return;

            Logger.error(`HiLo API call succeeded after ${retryCount} ${retryCount === 1 ? 'retry' : 'retries'} and ${totalTime} ms.`);
        }
    };
