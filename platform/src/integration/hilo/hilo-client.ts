...
export class HiloClient {
    ...
    public async restartGame(gameId: number): Promise<void> {
        return this.http.post(`admin/${gameId}/restart`);
    }

    public async updateGameState(gameId: number, state: number): Promise<void> {
        return this.http.post(`admin/${gameId}/state`, { state });
    }
}
