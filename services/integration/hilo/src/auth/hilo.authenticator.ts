...
import { HiloTokenVerifier } from './hilo.token-verifier';
import { Authenticator } from '@tcom/platform/lib/api/auth/authenticator';
import { LogClass } from '@tcom/platform/lib/core/logging';

@Singleton
@LogClass()
export class HiloAuthenticator implements Authenticator {
    constructor(
        @Inject private readonly parameterStore: ParameterStore,
        @Inject private readonly tokenVerifier: HiloTokenVerifier) {
    }
    ...
    public async execute(request: Request, scopes?: string[]): Promise<void> {
        ...
        const parameterKey = `/${Config.stage}/integration/hilo/operator-secret`;
