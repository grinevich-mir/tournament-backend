...
@Security('hilo')
@LogClass()
export class HiloController extends Controller {
    constructor(
    ...
    public async credit(@Body() request: GameCreditRequestModel): Promise<GameCreditResponseModel> {
        ...
        const walletEntry = await this.ledger.transfer(request.amount, request.currency)
            ...
            .requestedBy(RequesterType.System, `HiLo:${request.gameId}`)
            .externalRef(request.transactionId)
            .memo(`Pay Out for HiLo game ${request.gameId}`)
    ...
    public async gameRound(@Body() request: GameRoundRequestModel): Promise<void> {
        ...
        for (const token of Object.keys(request.results)) {
            ...
            if (tournament.leaderboardId) {
                const adjustment = this.createPointAdjustment(tournament, entry, result, request.gameMode);
                if (adjustment)
                    adjustments.push(adjustment);
            }

            if (result.complete && result.knockout) {
                const allocation = this.tournamentEntryManager.getCurrentAllocation(entry);
        ...
        if (tournament.leaderboardId && adjustments.length > 0) {
            const adjustmentResults = await this.leaderboardManager.adjustPoints(tournament.leaderboardId, ...adjustments);

            if (request.gameMode === GameMode.Roulette) {
                const breaches = adjustmentResults.filter(r => r.runningPoints > 2600);
                if (breaches.length > 0)
                    Logger.critical(`MAX SCORE BREACHED (2600) ON ASTRO SPINS (${request.gameId}): ${breaches.map(b => `${b.userId}: ${b.runningPoints}pts`).join(', ')}`);
            }
        }
    ...
    public async gameResult(@Body() request: GameResultRequestModel): Promise<void> {
        ...
        if (!request.results) {
            try {
                if (tournament.state <= TournamentState.Running)
                    await this.tournamentRuntimeManager.cancel(tournament);
            } catch (err) {
                Logger.error(err);
            }
            this.setStatus(200);
        ...
        for (const token of Object.keys(request.results)) {
            ...
            await this.ledger.transfer(amount, tournament.currencyCode)
                .purpose(TransactionPurpose.PayOut)
                .requestedBy(RequesterType.System, 'HiLo')
                .memo(`Pay Out for HiLo game ${request.gameId}`)
                .fromPlatform(PlatformWallets.Prize)
                .toUser(entry.userId, targetAccount)
                .commit();
    ...
    private createPointAdjustment(tournament: Tournament, entry: TournamentEntry, result: GameRoundResultModel, mode?: GameMode): LeaderboardAdjustment | undefined {
        ...
        switch (mode) {
            default:
                if (result.knockout && tournament.leaderboardPointMode === TournamentLeaderboardPointMode.Cumulative)
                    return;

                if (!result.knockout)
                    adjustment.points = result.points || 1;
                break;

            case GameMode.Roulette:
                adjustment.points = result.points || 0;
                break;
        }

        if (result.knockout && tournament.leaderboardPointMode === TournamentLeaderboardPointMode.Highest)
