...
import { WithdrawalRequestAddedEvent } from '@tcom/platform/lib/banking/events';
...
@Singleton
@LogClass()
export class OnWithdrawalRequestAddedHandler extends PlatformEventHandler<WithdrawalRequestAddedEvent> {
    constructor(
    ...
    public async process(event: WithdrawalRequestAddedEvent): Promise<void> {
        ...
        const header = `Withdrawal Requested (${Config.stage.toUpperCase()})`;
...
export const onWithdrawalRequestAdded = lambdaHandler((event: SNSEvent) => IocContainer.get(OnWithdrawalRequestAddedHandler).execute(event));